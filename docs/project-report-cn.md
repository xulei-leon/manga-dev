...
普遍认为 Sérsic 指数 $n = 2.5$ 是区分盘主导（晚型）和核球主导（早型）星系的临界值。我们以 Sérsic 指数 $n = 2.4$ 为界，将星系样本划分为高 $n$ 值（核球主导）和低 $n$ 值（盘主导）两个子样本，并分别对它们进行 $c-M_{200}$ 经验公式的拟合。结果表明，通过引入形态学分类，我们能够为 $c_0$ 和 $\alpha$ 参数获得误差更小、置信度更高的拟合结果。

具体而言，拟合结果显示：
高 $n$ 值具有更高的 $c_0$：$c_0$ 代表了在相同暗晕质量（如 $10^{12} M_\odot$）下的基准浓集度。
...
*图 4.4：星系的 $c-M_{200}$ 关系拟合结果。左图（a）展示了未区分Sérsic 指数的拟合情况 ；右图（b）展示了Sérsic 指数（$n < 2.4$，盘主导）子样本的拟合和高 Sérsic 指数（$n \ge 2.4$，核球主导）子样本分别拟合的情况。对比可见，高 $n$ 值星系具有显著更高的特征浓集度 $c_0$。*
